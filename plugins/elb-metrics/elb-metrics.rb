...
class ELBMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  option :metrics,
         ...
         short: '-m METRICS',
         long: '--metrics',
         default: 'Latency,RequestCount,UnHealthyHostCount,HealthyHostCount,SurgeQueueLength'
...
for metric in config[:metrics].split(',')
    begin
      ...
      if config[:statistic] == ''
        ...
        statistic = default_statistic_per_metric[metric]
      else
      ...
      unless data.nil?
        ...
        graphitepath = "#{config[:scheme]}.#{config[:elbname]}.#{metric.downcase}"
        if config[:scheme] == ''
          graphitepath = "#{config[:elbname]}.#{metric.downcase}"
        end
        # each time report metric with current timestamp
        output graphitepath, data[statistic.downcase.to_sym], Time.now.to_i
      end
    rescue => e
